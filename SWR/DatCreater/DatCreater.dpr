...
dtm1:=$0c5;
   ...
   begin
   pbyte(cardinal(ttmp5)+i)^:=dtm1 xor pbyte(cardinal(ttmp5)+i)^;
   dtm1:=(dtm1+dtm2) and $Ff;
   dtm2:=(dtm2+dtm3) and $Ff;
   ...
   for i := 0 to (size-1) do
   begin
  if mya>$26f then  updtable();



  ttmp:=pcardinal(@(mal[mya*4]))^;
  ttmp:=ttmp xor (ttmp shr $b);
  ttmp:=ttmp xor ((ttmp shl 7) and $9D2C5680);
  ttmp:=ttmp xor ((ttmp shl $F) and $0EFC60000);
  ttmp:= ttmp xor (ttmp shr $12);

  ttmp:=ttmp and $ff;

  pbyte(cardinal(ttmp5)+i)^:=ttmp xor pbyte(cardinal(ttmp5)+i)^;
   inc(mya);
...
for I := 0 to llst.Count - 1  do
  ...
  xrr:=((glosmesh shr 1) or $23) and $FF;
  ...
  tabsmesh:=tabsmesh+4;
  pbyte(cardinal(prs)+tabsmesh)^:=(length(prmbr(llst.Items[i]).v1) and $FF);
  tabsmesh:=tabsmesh+1;
